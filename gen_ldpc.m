...
%add bits_per_col 1's to each column with the only constraint being that the 1's
should be
...
%add 1's to rows having no 1(a redundant row) or only one 1(that bit in the
codeword becomes
...
%try to distribute the ones so that the number of ones per row is as uniform as
possible
for i=1:rows
j=1;
a=randperm(cols);
...
parity_check(i,coltoberearranged)=3; %make this entry 3 so that we dont
use
%of this entry again while getting
rid
%of other cylces
newrow=unidrnd(rows);
